...
// Localiza o elemento com o ID 'Consumables'
const spanElement = await page.$('#Consumables');

logger.info('found Consumables element')
...
logger.info('clicked on Consumables element show button')
...
tableData.slice(2).forEach(row => {
  ...
  // Salva os dados no nosso array de consumables.
  consumables.push({
    'name': name.trim(),
...
fileContentJson.consumables = consumables 
