...
// Localiza o elemento com o ID 'Artifacts'
const spanElement = await page.$('#Artifacts');

logger.info('found Artifacts element')
...
logger.info('clicked on Artifacts element show button')
...
tableData.slice(2).forEach(row => {
  ...
  // Salva os dados no nosso array de artifacts.
  artifacts.push({
    'name': name.trim(),
...
fileContentJson.artifacts = artifacts 
