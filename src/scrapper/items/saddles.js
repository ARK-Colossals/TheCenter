...
// Localiza o elemento com o ID 'Saddles'
const spanElement = await page.$('#Saddles');

logger.info('found Saddles element')
...
logger.info('clicked on Saddles element show button')
...
tableData.slice(2).forEach(row => {
  ...
  // Salva os dados no nosso array de saddles.
  saddles.push({
    'name': name.trim(),
...
fileContentJson.saddles = saddles 
