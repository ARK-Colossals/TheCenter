...
// Localiza o elemento com o ID 'Recipes'
const spanElement = await page.$('#Recipes');

logger.info('found Recipes element')
...
logger.info('clicked on Recipes element show button')
...
tableData.slice(2).forEach(row => {
  ...
  // Salva os dados no nosso array de recipes.
  recipes.push({
    'name': name.trim(),
...
fileContentJson.recipes = recipes 
